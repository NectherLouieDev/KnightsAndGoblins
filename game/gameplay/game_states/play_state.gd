class_name PlayState extends BaseGameState
...
@export_group("HUD")
@export var hud_spins_left: HUDSpinsLeft
...
func enter():
	...
	hud_spins_left.enter_complete_signal.connect(_on_hud_spins_left_enter)
	hud_spins_left.play_enter()
	
func _on_hud_spins_left_enter():
	hud_spins_left.enter_complete_signal.disconnect(_on_hud_spins_left_enter)
...
func _on_spawn_stagger_timer_timeout():
	...
	var type = result[i]
	spawn_point.spawn(i, type, enemy)
